...
**Claude Code가 권장 경로입니다.** 플러그인 호스트가 설치·업데이트·삭제를 관리합니다. push 명령을 실행 전에 검사하는 `PreToolUse` 가드 hook은 Claude Code 플러그인에만 포함됩니다. Codex와 Antigravity는 플러그인 시스템이 없어 `install.sh`가 파일을 복사합니다.
