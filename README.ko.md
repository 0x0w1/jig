...
사이드 프로젝트마다 AI 에이전트(Claude Code, Codex, Antigravity CLI)를 쓰다 보면, 브랜치 규칙·커밋 규칙·릴리즈 절차를 프로젝트마다·에이전트마다 다시 알려줘야 합니다. jig는 그 규칙을 한 번만 정하고, hook이 강제하고 버전으로 배포되는 형태로 고정합니다.

관습만이라면 — 브랜치 접두사, squash 병합, 커밋 형식 — 짧은 규칙 파일 하나로 충분하고, jig도 그 이상을 주장하지 않습니다. 규칙 파일에 담기지 않는 것은 세 가지입니다.

- **프롬프트에 적은 규칙은 부탁이고, hook에 넣은 규칙은 장벽입니다.** `PreToolUse` 가드가 push 명령을 실행 전에 읽어 툴 호출을 막고, 추적되는 `pre-push` hook이 git 층위에서 한 번 더 막습니다. 그것을 건너뛰려는 `--no-verify`까지 포함해서입니다. 규칙 파일로 옮겨 적을 수 있는 건 맨 위 층뿐입니다.
- **복사한 규칙에는 버전이 없습니다.** 저장소 열 곳에 붙여 넣은 규칙 파일은 어느 사본이 낡았는지 답하지 못합니다. jig 설치본은 버전 스탬프와 자신의 선택을 함께 들고 있고, `jig-doctor`가 해당 릴리즈의 파일 목록과 대조하며, `jig-update`가 찾아낸 모든 설치본을 각자의 선택을 보존한 채 수렴시킵니다.
- **릴리즈 등급은 습관이 아니라 파일에 있어야 합니다.** 문구를 고친 에이전트 지시문이 같은 입력에 여전히 발화하는지 증명하는 수단은 없습니다. 그래서 "사람이 봐야 한다"를 전달할 채널로 버전 번호가 남습니다. `.jig/versioning.md`가 그 판정 축을 커밋된 프로젝트 소유 문서로 붙잡습니다.

저장소가 여러 개이고 에이전트 CLI를 하나 이상 쓸 때 값이 납니다. 저장소가 하나라면 jig를 최신으로 유지하는 비용이 절차를 손으로 도는 비용보다 큽니다.
...
jig는 일반 스킬 모음과 달리 세션 절차만 배포하지 않습니다. 브랜치 모델·branch protection·릴리즈 규율이라는 *저장소 상태*를 함께 수렴합니다. 하나의 절차 원본(`skills/`)을 각 CLI의 네이티브 형식으로 렌더링합니다 — Claude Code는 플러그인, Codex와 Antigravity는 `jig-` prefix 파일입니다.
