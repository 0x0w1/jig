...
7. Update every verified Claude Code standalone root with the helper shipped by the latest release:
   ...
   - The helper re-verifies the root ledger and every selected directory's provenance, supports both unprefixed and `jig-` prefixed directory names, and writes the version ledger for a verified legacy installation after its first successful update.
