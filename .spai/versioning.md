...
SPAI는 다른 저장소에 설치되는 도구다. 릴리즈 노트의 주 소비자가 사람이 아니라 `spai-update` 스킬이므로, 에이전트가 무인으로 처리할 수 있는 변경과 사람의 판단이 필요한 변경을 같은 등급으로 묶지 않는다.

이 파일이 SPAI 릴리즈 판정의 규범 원본이다. `github-release`가 릴리즈할 때 읽는다. 사람이 읽는 해설과 적용 예는 [docs/versioning.md](../docs/versioning.md)에 있다.

## 판정 순서

1. 설치본이 아무것도 하지 않아도 되거나, 업데이트 재실행만으로 수렴하는가? → `patch`
2. 깨지긴 하지만 (a) `spai-update`가 무인으로 복구하거나 (b) 실패 지점에서 대체 수단을 즉시 제시하는가? → `minor`
...
> 릴리즈 노트에 `migration-manual` 블록이 하나라도 있으면 `major`다. `migration-auto` 블록만 있으면 최소 `minor`다. 블록은 줄 전체가 marker일 때만 센다(`^<!-- spai:start migration-manual -->$`).
...
1. installer와 설치 후 GitHub 프로필 계약: `--target`, `--scope`, 선택 옵션 `--github-profile`/`--github-account`, `SPAI_GITHUB_PROFILE`, `spai.githubProfile`
2. 스킬 호출 이름: `/spai:<skill>`, `spai-<skill>`
3. 플러그인·마켓플레이스 이름: `spai@spai`, `0x0w1/spai`
4. managed block marker 문자열: `<!-- spai:start ... -->`, `<!-- spai:end ... -->`
5. 저장소 모델: 브랜치 이름, 병합 흐름, 보호 정책
6. 버전 판정 기준 계약: `.spai/versioning.md` 경로, `SPAI_VERSION_RUBRIC`, `spai.versionRubric`, 섹션 제목 `## 판정 순서`·`## 등급 정의`
