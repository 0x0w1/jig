...
- This source repository has a local maintenance rule, not a capability shipped to installed projects: every manifest skill has matching guides at `docs/en/skills/<name>.md` and `docs/ko/skills/<name>.md`. When any file under `skills/<name>/` changes, review both guides and update every affected workflow, input/output, safety rule, resource, and Mermaid diagram. Then run `sh scripts/update-skill-doc-digests.sh <name>`; validation rejects stale guide digests. Skill additions and removals also update both language indexes.
...
- Do not create unrequested AI skill directories beyond those two inside this repository. `.codex/hooks.json` is written only by `jig-github-sync`'s native hook manager.
...
- Codex installs the `jig` plugin (`codex plugin marketplace add 0x0w1/jig`, `codex plugin add jig@jig`) from the same payload as Claude Code; `install.sh` targets only Antigravity, and `dist/codex/` no longer exists. A `.agents/skills/jig-*` Codex installation is legacy and migrates to the plugin by hand.
...
- A hotfix must name a matching item from `## 핫픽스 트리거` in `.jig/versioning.md`; `jig-hotfix-flow` stops when nothing matches, and widening that list is a separate committed decision rather than a step inside a hotfix.
