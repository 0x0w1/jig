...
description: Use when Codex needs to add durable project knowledge into an external LLM + Obsidian wiki + Graphify vault. Triggers include requests to ingest raw knowledge, send notes into a knowledges project, configure KNOWLEDGES_ROOT from .env, inspect a knowledges/raw tree, or prepare raw Markdown for later raw -> wiki -> graphify sync.
---
