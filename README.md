...
- 릴리즈 노트는 릴리즈 범위의 `develop` 대상 작업 PR에서 취합되므로, 모든 변경은 `feature/*`, `fix/*`, `chore/*` PR로 `develop`에 병합되어야 노트에 나타납니다.
...
- `github-release`: base `main`, head `develop`인 릴리스 PR을 통해 `develop`을 `main`으로 승격하고 Release Drafter 게시를 진행합니다.
...
- `develop` 대상 작업 브랜치
  ...
  - `fix/<slug>`
  - `chore/<slug>`
- `develop`에서 `main`으로의 릴리스 PR (별도 `release/*` 브랜치 없음)
- 정확히 6개의 표준 라벨
  - `patch`
  - `minor`
  - `major`
