...
원본 편집 파일의 레이어 구성이나 작업 방식은 내보낸 자산이 같은 한 약속이 아닙니다.

## 판정 순서

1. 기존 이름·포맷·의미를 유지하며 자산을 다듬거나 추가했는가? → `patch`
2. 기존 자산을 그대로 두고 변형·포맷·토큰을 추가했는가? → `minor`
...
| bump | 정의 | 예 |
|---|---|---|
| `patch` | 참조를 그대로 둔 자산 수정 | 아이콘 픽셀 정렬, 여백 정리, 파일 용량 최적화, 새 아이콘 추가 |
| `minor` | 기존 자산과 공존하는 확장 | 다크 모드 변형 추가, 새 포맷 내보내기, 토큰 추가 |
