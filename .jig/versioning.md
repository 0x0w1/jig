...
SPAI는 다른 저장소에 설치되는 도구다. 릴리즈 노트의 주 소비자가 사람이 아니라 `jig-update` 스킬이므로, 에이전트가 무인으로 처리할 수 있는 변경과 사람의 판단이 필요한 변경을 같은 등급으로 묶지 않는다.
...
3. 플러그인·마켓플레이스 이름: `jig@jig`, `0x0w1/spai` (저장소 슬러그는 아직 `spai`)
