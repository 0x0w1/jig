...
jig는 다른 저장소에 설치되는 도구다. 설치본이 이미 받아 간 payload가 망가졌을 때만 `develop` 대기열을 건너뛴다.

- installer가 실패하거나, 설치된 스킬이 로드되지 않는다
- `jig-update`가 설치본을 이전 상태로 되돌리지 못한 채 중단된다
- 배포된 가드가 막아야 할 push를 막지 못한다
- 설치본의 파일이나 사용자 hook이 손실된다
- 공개된 보안 취약점이 악용 가능하다

목록에 없으면 위 질문으로 판단한다. 답이 `아니오`면 핫픽스가 아니다. 답이 `예`면 그 조건을 이 목록에 추가하고 따로 커밋한 뒤에 핫픽스를 시작한다. 급한 느낌, 마감, 데모는 답이 아니다 — 일정에 대한 서술이지 릴리즈된 상태가 계속 하는 일이 아니다.
