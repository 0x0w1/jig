...
grep -F '<!-- jig:version custom skills=jig-setup -->' "$TEST_ROOT/project/GEMINI.md" >/dev/null

if (
  cd "$TEST_ROOT/project"
  REPO_RAW_URL="file://$REPOSITORY_ROOT" \
    sh "$REPOSITORY_ROOT/install.sh" --target codex --scope project \
    > "$TEST_ROOT/codex.log" 2>&1
); then
  echo "installer skill alias tests: --target codex was accepted" >&2
  exit 1
fi
grep -F 'codex plugin add jig@jig' "$TEST_ROOT/codex.log" >/dev/null
test ! -e "$TEST_ROOT/project/AGENTS.md"
