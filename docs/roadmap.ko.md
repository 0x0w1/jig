...
v0.2.0 이전에는 배포 payload에 `<!-- spai:owned skill=<name> -->` 마커를 넣었다. installer는 마커 없는 파일을 건너뛰고 버전 스탬프를 소유권 대장으로 삼아 구버전을 승계하고 선택에서 빠진 파일은 고아로 보고했다. 플러그인 네임스페이스(Claude Code)와 `spai-` prefix(Codex/Antigravity)로 대체하면서 전부 제거했다. 호스트가 제공하는 기능을 재구현하지 않는다는 원칙의 사례로 남긴다.
