...
카탈로그 파일은 그대로 `.jig/versioning.md`가 될 수 있게 쓰여 있습니다. 복사한 뒤 `> Basis:` 줄의 날짜와 `## Public Interface` 목록, 그리고 `## Interface Paths`의 glob을 프로젝트에 맞게 고칩니다. glob은 그 유형이 관례적으로 쓰는 배치를 적어둔 것이라 이 저장소의 실제 트리와는 다를 수 있습니다. 카탈로그 자체는 payload라서 `jig-update`가 갱신합니다 — 프로젝트의 결정은 카탈로그가 아니라 기준 파일에 적습니다.
...
설치 직후에는 `jig-setup`이 이 스킬을 대신 호출합니다. 릴리즈 시점에 파일이 없으면 `github-release`가 호출한 뒤 릴리즈를 계속 진행합니다. 질문에 답이 없으면 쓰지 않고 제안으로 남깁니다. 기본 기준 생성은 사용이 명시적으로 승인된 경우에만 가능하며, 기존 기준 교체에는 구체적 교체 결정이 필요합니다. 호출 스킬도 이 제한을 따르며 필요한 기준이 이미 생긴 것으로 가정하지 않습니다.
...
- 카탈로그의 모든 유형 초안이 이 표를 갖고 있어서, 유형을 채택하면 경로 바닥이 함께 켜집니다. 기본 rubric은 표가 없습니다 — 사람 개입 축은 경로를 지목하지 않기 때문이며, 바닥이 필요하면 `version-rubric`에 표를 덧붙여 달라고 하면 됩니다.
