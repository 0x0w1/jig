...
위 목록은 산문이라 누군가 읽어야만 작동한다. `.jig/versioning.md`에는 같은 약속을 path glob으로 옮긴 `## 인터페이스 경로` 표가 함께 있고, `develop-task-flow`와 `github-release`가 `git diff --name-only` 결과를 여기에 대조한다. 아래는 형태를 보여주는 일부이며 전체 표는 규범 원본에 있다.
...
| `skills/**` | 스킬 본문 | `patch` |
| `dist/**`, `scripts/**`, `docs/**` | 내부 | `patch` |

바뀐 경로는 **처음 걸리는 행**의 바닥을 갖고, 범위의 바닥은 그중 최대값이다. **이 바닥은 참고값이다.** 경로는 무엇이 바뀌었는지만 말하고 어떻게 바뀌었는지는 말하지 않으므로, 이유를 남기면 바닥보다 낮게 낼 수 있다. `install.sh` 안의 주석 수정은 여전히 `patch`다. 이 표의 역할은 명백한 누락을 막는 것이지 판정을 대신하는 것이 아니다.

## 병합 시점에 기록하는 등급

`develop-task-flow`는 squash 병합을 하면서 그 task의 등급을 판정하고 커밋 본문에 trailer로 남긴다.

```text
Release-Grade: minor
```

병합 시점에는 diff와 테스트 결과가 아직 손에 있지만, 릴리즈 시점에는 커밋 텍스트로 그것을 재구성해야 한다. `github-release`는 범위 안에 기록된 등급 중 최고값을 릴리즈 하한으로 삼고, 그것을 정한 커밋을 보고에 적는다.

경로 바닥과 달리 **기록된 등급은 내리지 않는다.** 릴리즈 시점에는 더 이상 없는 근거로 내린 판단이기 때문이다. trailer가 없는 커밋은 이전과 똑같이 제목과 본문으로 판정하므로, 이 규칙이 생기기 전의 이력은 판정 결과가 달라지지 않는다.
