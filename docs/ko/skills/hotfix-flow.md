...
실행할 때는 맞은 조건의 이름을 대고 squash commit에 `Hotfix-Trigger:` trailer로 기록합니다. 맞는 것이 없으면 기준 질문을 묻습니다. 답이 `아니오`면 실행을 멈추고 `develop-task-flow`로 넘기며, `예`면 목록에 조건 하나가 빠져 있다는 뜻이므로 그 조건을 절에 추가해 따로 커밋한 뒤 수정을 시작합니다. 목록은 예외가 아니라 결정으로 자라고, 자신이 막고 있는 실행 안에서는 자라지 않습니다.
