...
> This document is the human-readable commentary. The normative source for grading a SPAI release is [`.spai/versioning.md`](../.spai/versioning.md), and that is the file `github-release` reads at release time. The two must say the same thing.
>
> SPAI installed in another project reads *that* project's `.spai/versioning.md`. How to create or re-settle a rubric is in [version rubric](version-rubric.md).

A SPAI `vX.Y.Z` bump is decided by **what an installation actually pays**, not by the kind of change it was. The main consumer of the release notes is the `spai-update` skill rather than a person, so a change an agent can finish unattended and a change that needs a human decision never share a grade.
...
2. Does something break, but either (a) `spai-update` repairs it unattended, or (b) **the failure names its own replacement at the point of failure**? → `minor`
...
| `minor` | A new capability, or a **guided break**. | new skill or target, a removed option that fails while naming its replacement, a skill directory rename `spai-update` migrates for you |
...
1. The curl one-liner and the post-install GitHub profile contract: `--target`, `--scope`, the optional `--github-profile`/`--github-account`, `SPAI_GITHUB_PROFILE`, `spai.githubProfile`
2. Skill invocation names: `/spai:github-release`, `spai-github-release`
3. Plugin and marketplace names: `spai@spai`, `0x0w1/spai`
4. Managed block marker strings: `<!-- spai:start ... -->`, `<!-- spai:end ... -->`
...
The `### Migration` section is not prose. It is written as two kinds of marker block, because it is input `spai-update` executes rather than a paragraph a person reads.
...
|---|---|---|
| `migration-auto` | A mechanical step an agent finishes unattended | `spai-update` runs it |
| `migration-manual` | A step that needs human judgement | `spai-update` presents it and **does not run it without approval** |
...
| Release | Grade | Why |
|---|---|---|
| `v0.2.0`, the move to a plugin | `major` | The invocation name changed from `/github-release` to `/spai:github-release` while files copied by the older version stayed behind quietly, so behavior forked. Nothing failed |
