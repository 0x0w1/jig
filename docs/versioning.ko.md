...
> 이 문서는 사람이 읽는 해설입니다. SPAI 릴리즈 판정의 규범 원본은 [`.spai/versioning.md`](../.spai/versioning.md)이고 `github-release`가 릴리즈할 때 읽는 파일도 그쪽입니다. 두 파일은 같은 내용을 말해야 합니다.
>
> 다른 프로젝트에 설치된 SPAI는 그 프로젝트의 `.spai/versioning.md`를 읽습니다. 판정 기준을 만들거나 다시 잡는 방법은 [버전 판정 기준](version-rubric.ko.md)에 있습니다.

SPAI의 `vX.Y.Z` bump는 **변경의 종류**가 아니라 **설치본이 실제로 치르는 비용**으로 결정합니다. 릴리즈 노트의 주 소비자가 사람이 아니라 `spai-update` 스킬이기 때문에 에이전트가 무인으로 처리할 수 있는 변경과 사람의 판단이 필요한 변경을 같은 등급으로 묶지 않습니다.
...
2. 깨지긴 하지만 (a) `spai-update`가 무인으로 복구하거나 (b) **실패 지점에서 대체 수단을 즉시 제시**하는가? → `minor`
...
| `minor` | 새 기능, 또는 **유도된 파괴(guided break)**. | 새 스킬·새 target 추가, 제거된 옵션이 대체 명령을 안내하며 실패, 스킬 디렉토리 rename을 `spai-update`가 자동 이관 |
...
1. curl 원라이너와 설치 후 GitHub 프로필 계약: `--target`, `--scope`, 선택 옵션 `--github-profile`/`--github-account`, `SPAI_GITHUB_PROFILE`, `spai.githubProfile`
...
**marker는 줄 전체일 때만 marker입니다** (`^<!-- spai:(start|end) migration-(auto|manual) -->$`). 릴리즈 노트는 이 marker 이름을 본문에서 자주 언급하므로 백틱 안이나 문장 중간에 나온 것은 그냥 텍스트입니다. 부분 문자열로 세면 본문 언급까지 블록으로 오인합니다. 열린 블록에 짝이 되는 end marker가 없으면 결함으로 보고하고 아무것도 실행하지 않습니다.
...
| 릴리즈 | 판정 | 근거 |
|---|---|---|
| `v0.2.0` 플러그인 전환 | `major` | 호출 이름이 `/github-release` → `/spai:github-release`로 바뀌었는데 구버전이 복사한 파일이 조용히 남아 동작이 갈림. 실패하지 않는 변경 |
